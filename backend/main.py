# main.py - *routes and coordinates*, the backend controller for the app
...
from outfit_selector import generate_outfits
...
async def upload_clothing(image: UploadFile = File(...)):
    ...
    clean_item = normalize_item(ai_tags)
...
def generate_outfit(request: OutfitRequest):
    ...
    # Generate outfits - filters invalid items(rules), scores items(AI logic), generates 3 outfits, avoids repeats
    ...
    accessories = suggest_accessories(outfits, request.occasion.value, request.preferences)

    makeup = suggest_makeup(outfits, request.occasion.value, request.preferences)

    # Uses OpenAI to explain why the outfits were chosen
